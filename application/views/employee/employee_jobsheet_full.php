           ...
           <aside class="right-side  <?php if(isset($left_menu) && $left_menu==0){ echo "strech"; }?>">                
                ...
                <?php echo $message_div ?>
                    <div class="row">
					  ...
					  <form style="width: 100%;overflow-x: scroll;" role="form" id="jobsheet_full" action="<?php echo  site_path?>employee_jobsheet/jobsheet_complete" method="post">
										...
										//print_r($total_list);
											if($total_list!=false)
											{
												...
												foreach($total_list as $list)
												{
													...
													echo "<td><input ".$leave_disabled." type='input' class='form-control normal' onkeyup='calculate_total(this);' value='".$normal_hour."' required id='normal_".$list['emp_id']."' name=\"normal_hours[".$list['emp_id']."]\"></td>";
													echo "<td><input value='".$fixed_hours."' type='input'  ".$leave_disabled."  class='form-control' onkeyup='calculate_total(this);' id='otf_".$list['emp_id']."' name=\"otf[".$list['emp_id']."]\"></td>";
													echo "<td><input value='".$ot1."'  ".$leave_disabled."  type='input' class='form-control'  onkeyup='calculate_total(this);' id='ot1_".$list['emp_id']."' name=\"ot15[".$list['emp_id']."]\"></td>";
													?>
                                                    
                                                    <?php
													
													echo "<td><input  value='".$ot2."' ".$leave_disabled."  type='input' class='form-control' onkeyup='calculate_total(this);' id='ot2_".$list['emp_id']."'  name=\"ot2[".$list['emp_id']."]\"></td>";
            ...
            <div class="modal" id="modal1">
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form role="form" id="absent" action="<?php echo  site_path?>employee_jobsheet/absent" method="post">
                                 <input type="hidden" name="<?php echo  $this->security->get_csrf_token_name(); ?>" id="csrf_token" value="<?php echo  $this->security->get_csrf_hash(); ?>"/>
         ...
         $(document).ready(function(){
            ...
            hourFormat: "24"
         	...
         	$( "#absent" ).on( "submit", function( event ) {
        		...
        		var id=$('#hidden_uid').val();
        		   ...
        		   $.ajax({
				type: "POST",
				url: "<?php echo site_path; ?>employee_jobsheet/absent",
				dataType: "json",
			...
			$('.leave').change(function(){
					...
					else
					{
						...
						if(s)
						{
						 ...
						 $.ajax({
								type: "GET",
								url: "<?php echo site_path; ?>employee_jobsheet/reabsent",
								dataType: "json",
			...
			$( "#jobsheet_full" ).on( "submit", function( event ) {
        		 ...
        		 $('#error_log').html("");
        		   $.ajax({
				type: "POST",
				url: "<?php echo site_path; ?>employee_jobsheet/jobsheet_complete",
				dataType: "json",
				...
				else if(html.status==1)
				{
					...
					window.location.href='<?php  echo site_path; ?>employee_jobsheet/';
