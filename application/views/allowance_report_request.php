            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1>
                        Allowance Request
                        <small>For <span class="label label-warning"></span></small>
                    </h1>
                    <ol class="breadcrumb">
											...
											<?php
												...
												if($allowance_list!=false)
												{
													foreach($allowance_list as $employee)
													{
														...
														echo '<tr><td>'.$employee_no.'</td><td>'.$employee_name.'</td><td>'.$employee->allowance_type_name.'</td><td>'.$employee->emp_allowance_amount.'</td><td>'.$from.' - '.$to.'</td><td>'.$month.'</td><td><a href="javascript:void(0);" onclick="open_modal('.$employee->emp_allowance_id.')"><span class="label label-warning"><i class="fa fa-trash-o"> </i> Approve/Reject</a></span></a></td></tr>';
													}
												}
												else
												{
			...
			$.ajax({
				...
				else if(html.status==1)
				{
					$.each(html.result, function(k,v) {
           			...
           			//alert(html.result.emp_allowance_filename);
