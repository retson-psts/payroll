			...
			if($type>3)
			{
				...
				if($type==4 || $type==5)
				{
					...
					$to=$from_date->format('Y-m-t');
						...
						{
							$month=explode('-',$date);
							$check_already_generated=$this->common_model->__fetch_contents('salary_master',array('month(salary_master_month)'=>$month[1],'year(salary_master_month)'=>$month[0],'salary_master_removed'=>0,'salary_pay_period'=>$type),'count(salary_master_id) as generated');
				...
				if($type==6)
				{
					...
					if($check_data==1)
					{
						$check_already_generated=$this->common_model->__fetch_contents('salary_master',array('salary_period_from'=>$from,'salary_period_to'=>$to,'salary_master_removed'=>0,'salary_pay_period'=>$type),'count(salary_master_id) as generated');
