...
class Employee_jobsheet extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('employee/jobsheet_view',$data);
	...
	public function date_jobsheet()
	{
		...
		else
		{
			...
			if(count($date1)==3 && checkdate($date1['1'],$date1['2'],$date1['0']))
			{
				...
				if($today>=$date)
				{
					$checkadmin_added=$this->common_model->fetch_contents('jobsheet_master',array('jobsheet_master_date'=>$date_follow));
					//var_dump($checkadmin_added);
					if($checkadmin_added===false)
					{
						$data['jobsheet_master_id']=0;
						$check_user_added=$this->common_model->fetch_contents('employee_jobsheet_master',array('jobsheet_master_date'=>$date_follow,'jobsheet_master_entered_by'=>$user_id));
						...
						$this->load->view('header',$data);
						$this->load->view('employee/side_menu_employee',$data);
						$this->load->view('employee/employee_jobsheet_full',$data);
						$this->load->view('footer');
					}
					else
					{
						$message="You can't update after admin updated";
						$data=array('status'=>'0','message'=>$message,'form_data'=>array('jobsheet_date'=>$this->input->post('jobsheet_date')));
						$this->session->set_userdata('form',array('jobsheet'=>$data));
						redirect(site_path.'employee_jobsheet','refresh');
	...
	public function jobsheet_complete()
	{
		...
		if($dublicate===false)
		{
			$insert_master=$this->jobsheet_model->jobsheet_complete_employee($master,$workingarray,$arrayjobsheet);
