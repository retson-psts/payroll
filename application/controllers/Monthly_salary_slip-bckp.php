			...
			if($type>3)
			{
				...
				if($type==4 || $type==5)
				{
					...
					$to=$from_date->format('Y-m-t');
						...
						{
							$month=explode('-',$date);
							$check_already_generated=$this->common_model->__fetch_contents('salary_master',array('month(salary_master_month)'=>$month[1],'year(salary_master_month)'=>$month[0],'salary_master_removed'=>0,'salary_pay_period'=>4),'count(salary_master_id) as generated');
				...
				if($type==6)
				{
					...
					if($check_data==1)
					{
						$check_already_generated=$this->common_model->__fetch_contents('salary_master',array('salary_period_from'=>$from,'salary_period_to'=>$to,'salary_master_removed'=>0,'salary_pay_period'=>6),'count(salary_master_id) as generated');
	...
	*/
	private function generate_salary_slip($month,$from='',$to='',$type=4)
	{
		...
		foreach($fetch_employee_id as $employee_id)
		{
			...
			$final_array[$employee_id['employee_id']]['deduction_details']=json_encode($deduction_details[$employee_id['employee_id']]);
			$final_array[$employee_id['employee_id']]['leaves_details']=json_encode($leaves_details[$employee_id['employee_id']][0]);
			$final_array[$employee_id['employee_id']]['allowance_details']=json_encode($allowance_details[$employee_id['employee_id']][0]);
			$final_array[$employee_id['employee_id']]['jobsheet_details']=json_encode($jobsheet_details[$employee_id['employee_id']][0]);
			//var_dump($deduction[$employee_id['employee_id']][0]);
