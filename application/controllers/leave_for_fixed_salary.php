...
class Leave_for_fixed_salary extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('leave_for_fixed_salary_view',$data);
	...
	public function leave_list()
	{
		...
		{
			if($this->form_validation->run('request_fixed_leave')!==false)
			{
	...
	public function employee_leave_add()
	{
		if ($this->input->is_ajax_request())
		{
			if($this->form_validation->run('emp_fixed_leave_add')!==false)
			{
				...
				foreach($dates as $date)
				{
					...
					$leave_data=array('employee_id'=>$employee_id,'leave_date'=>$date,'leave_type'=>$leave_types[$i],'leave_deduct_salary'=>$deduct_salary,'leave_request'=>1,'leave_accept_by'=>$this->session->userdata['logged_in']['user_id'],'leave_entered_by'=>$this->session->userdata['logged_in']['user_id']);
				...
				{
					$leave_request=array('leave_request_user_id'=>$employee_id,'leave_request_type'=>$data['leave_type'],'leave_request_from'=>$from,'leave_request_to'=>$last,'leave_notes'=>$data['leave_notes'],'leave_requested_at'=>date('Y-m-d H:i:s'),'leave_request_approve_status'=>1,'leave_request_approved_by'=>$this->session->userdata['logged_in']['user_id']);
	...
	public function check_leave()
	{
		if ($this->input->is_ajax_request())
		{
			if($this->form_validation->run('check_leave_fixed')!==false)
			{
	...
	public function search_fixed_employees()
	{
		if ($this->input->is_ajax_request())
   		{
   			$get=$this->input->get(NULL,TRUE);
   			$term=$get['term'];
   			$emp_list=$this->leave_model->fixed_salary_employees($term);
   			$list_array=array();
   			if(!empty($emp_list))
   			{
